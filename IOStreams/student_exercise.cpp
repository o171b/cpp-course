...
//write a program to store an retrieve list of item in a file using serialization
...
public:
   ...
   friend ifstream & operator>>(ifstream &fis, Item &i); //insertion operator
   friend ofstream & operator<<(ofstream &fos, Item &i); //extraction operator
   friend ostream & operator<<(ostream &os, Item &i); //insertion operator on console, that is output stream operator
};
